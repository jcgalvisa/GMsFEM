...
function square_mesh(nx, ny, Nx, Ny, intx, inty)
    ...
    vup = collect((ne_x + 1) * ne_y + 1:(ne_x + 1) * (ne_y + 1))
    vdown = collect(1:(ne_x + 1))
    vleft = collect(1:(ne_x + 1):(ne_x + 1) * (ne_y + 1))
    vright = collect((ne_x + 1):(ne_x + 1):(ne_x + 1) * (ne_y + 1))
...
#	println(P)
    ...
    ub_x = zeros(4, length(txi))
    ub_y = zeros(4, length(txi))
...
P = reshape(P, (9, 2))
    ...
    function_fl = forcing(xi, eta)
    L = zeros(4)
    for i in 1:4
        L[i] = sum(Omega.* ub[i, :] .* function_fl)
    end
    L = L * hx / 2 * hy / 2
    return L
...
function Nmatrix(M::Matrix{Int64}, v::Matrix{Float64}, coef::Vector{Float64}, mesh::Dict{Symbol, Real})
    nvel = Int(mesh[:nv])
    A = zeros(nvel, nvel)
    b = zeros(nvel)
...
mutable struct Neighborhood
    ...
    elements::Vector{Int}
    c::Vector{Float64}
    diam::Dict{Symbol, Float64}
	psibad::Array{Float64, 2}
	Nbad::Int
    lambda::Vector{Float64}
    philin::Vector{Float64}
    cb::Array{Float64, 2}
end

function createmeshomegai(nx, ny, Nx, Ny, I1, I2)
    Cnx = 2
    Cny = 2
    if I1 == 1 || I1 == Nx + 1
        Cnx = 1
    ...
    for iy in 1:ney
        for ix in 1:nex
            ne += 1
            Ilocalx = (I1-2 + (I1 == 1)) * nx + ix
            Ilocaly = (I2-2 + (I2 == 1)) * ny + iy
            ige = (Nx * nx) * (Ilocaly - 1) + Ilocalx
            push!(Ie, ige)
        end
    end
    element_list_omega = Ie

    return nodes_list_omega, element_list_omega
end

function neighborhoods(ax, bx, ay, by, Nx, Ny, nx, ny)
#    println("Setting coarse neighborhood info...")
    neigh = Array{Neighborhood}(undef, Nx + 1, Ny + 1)

    for i1 in 1:Nx+1
        for i2 in 1:Ny+1
            nodes_list_omega, element_list_omega = createmeshomegai(nx, ny, Nx, Ny, i1, i2)
            c = [ax + (i1 - 1) * (bx - ax) / Nx, ay + (i2 - 1) * (by - ay) / Ny]
            diam = Dict(:x => 2 * (bx - ax) / Nx, :y => 2 * (by - ay) / Ny)
            neigh[i1, i2] = Neighborhood(nodes_list_omega, element_list_omega, c, diam,zeros(2,2),1,[],[],zeros(2,2))
        end
    end
...
# ╔═╡ 9c04e6d0-101e-4a57-bf19-bb4606aaf2c7
md""" # Local eigenvalue problem"""
...
function NMassmatrix(M::Matrix{Int64}, v::Matrix{Float64}, coeff::Vector{Float64}, mesh::Dict{Symbol, Real})
    ...
    for i in 1:size(M, 1)
        ...
        lcol = Int.(M[i, 1:4]) 
        lA = localMass(M[i, 1:4], v)
        A[lcol, lcol] .+= coeff[M[i,5]] * lA
...
#    println("neighborhood stiffness and mass matrix...")
#    println("local eigenvalue problems...")
   Aaux=0;
    for i1 in 1:Nx+1
        for i2 in 1:Ny+1
 ...
 #           println("i1: $i1, i2: $i2")
 #           println("nodes_i: ", nodes_i)
 #           println("elements_i: ", elements_i)

            Ai = Nmatrix(Elements[elements_i, :], vertex_list,coefficient_values,mesh_parameter)
            Mi = NMassmatrix(Elements[elements_i, :], vertex_list, coefficient_values, mesh_parameter)

            Ai_matrix = Ai[1]
            Mi_matrix = Mi[1]

  #          println("Ai_matrix: ", size(Ai_matrix))
 #           println("Mi_matrix: ", size(Mi_matrix))

            Ai_submatrix = Ai_matrix[nodes_i, nodes_i]
            Mi_submatrix = Mi_matrix[nodes_i, nodes_i]

#            println("Ai_submatrix: ", size(Ai_submatrix))
#            println("Mi_submatrix: ", size(Mi_submatrix))

            D, Q = eigen(Array(Ai_submatrix), Array(Mi_submatrix))
#            println("Q: ", size(Q))
#            println("D: ", size(D))
            ...
            incre = diff(lambda)
            Iincre = abs.(incre) .> .1 # L_i computed automaticaly.
  ...
  #          println("Size of Q: ", size(Q))
            ...
            n_cols_Q = size(Q, 2)
            if n_cols_Q >= (Nbad + 1)
 #               println("Assigning psi1 with Q[:, 1:(Nbad + 1)]")
                psi1[nodes_i, 1:(Nbad + 1)] = Q[:, 1:(Nbad + 1)]
            else
 #               println("Q does not have enough columns.")
 #               println("Reducing Nbad to ", n_cols_Q - 1)
                Nbad = n_cols_Q - 1
#                println("Assigning psi1 with Q[:, 1:(Nbad + 1)]")
				psi1[nodes_i, 1:(Nbad + 1)] = (Q[:, 1:(Nbad + 1)])
...
function linearpu(neigh, Nx, Ny, b, vertex_list)
	aux=0
    for i1 in 1:Nx+1
        for i2 in 1:Ny+1
            xlin = b*0
...
linearpu(neigh,Nx,Ny,bfinegrid,vertex_list)
...
function GMsFEMbasis(neigh::Matrix{Neighborhood}, Nx::Int, Ny::Int)
    ...
    for i1 in 1:Nx+1
        for i2 in 1:Ny+1
			...
			aux= neigh[i1, i2].Nbad
			cb1=zeros(size(neigh[i1,i2].psibad))
#			returnaux=cb1
            for l in 1:Nbad
...
for i1=1:Nx+1
    for i2=1:Ny+1
        ...
        for l=1:NbadI
            I=I+1;
            coarsebasis=neigh[i1,i2].cb[:,l];
           if i1==1 || i1==Nx+1|| i2==1|| i2==(Ny+1)
#            if i1==1 || i1==Nx+1
                if l==1
                    dirn=vcat(dirn,I);
                    X=vcat(X,neigh[i1,i2].c[1]);
                    Y=vcat(Y,neigh[i1,i2].c[2]);
                    Ig=neigh[i1,i2].nodes;
...
# ╔═╡ 34092e0f-ccac-41aa-825d-2c54a3477b4d
md"""
# References
[1] Galvis, J., & Efendiev, Y. (2010). Domain decomposition preconditioners for multiscale flows in high-contrast media. Multiscale Modeling & Simulation, 8(4), 1461-1483.

[2] GALVIS, Juan; Efendiev, Yalchin (2010). Domain decomposition preconditioners for multiscale flows in high contrast media: reduced dimension coarse spaces. Multiscale Modeling & Simulation, 2010, vol. 8, no 5, p. 1621-1644.

[3] Efendiev, Y., Galvis, J., & Hou, T. Y. (2013). Generalized multiscale finite element methods (GMsFEM). Journal of computational physics, 251, 116-135.

[4] Abreu, E., Díaz, C., & Galvis, J. (2019). A convergence analysis of generalized multiscale finite element methods. Journal of Computational Physics, 396, 303-324.
"""
