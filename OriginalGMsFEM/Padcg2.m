function [x, error, iter, flag, lambdamax, condnumber] = Padcg2(x, b,br, max_it, tol,dom,dom_ov,Nx,Ny)
...
%                           1 = no convergence given max_it
  ...
  %xaux=applay_A(x,dom,Nx,Ny);
  Ax=applay_Aadd_inv2B(x,dom_ov,Nx,Ny);
%  r=0*b;
  r = b -Ax;
...
epsilon=0.00001;
      z  =  applay_AN(r,dom,Nx,Ny)+(epsilon*r);
%      z  = pc(r,iter);
     rho = (r'*z);
...
%      paux=applay_A(p,dom,Nx,Ny);
     Ap=applay_Aadd_inv2B(p,dom_ov,Nx,Ny);
%     q=0*p;
     q=Ap;
     alpha(iter) = rho / (p'*q );
