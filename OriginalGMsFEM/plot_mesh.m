...
for i=1:Nx
    for j=1:Ny
        ...
        v=dom(i,j).v;
        if(mod((i+j),2)==0) 
           triplot(M,v(:,1)+(i-1)*e,v(:,2)+(j-1)*e,'red','LineWidt',2);
%             triplot(M,v(:,1)+(i-1)*e,v(:,2)+(j-1)*e,'red');
        else
           triplot(M,v(:,1)+(i-1)*e,v(:,2)+(j-1)*e,'green','LineWidt',2);
%             triplot(M,v(:,1)+(i-1)*e,v(:,2)+(j-1)*e,'blue');
        end
    hold on
    end
end
