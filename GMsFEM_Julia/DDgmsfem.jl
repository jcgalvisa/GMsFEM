...
begin
	K = ones(Nx*nx, Ny*ny)
	K[nx:(Nx-1)*nx, nx:(nx+1)].=1000
	K[nx:(Nx-1)*nx, (nx+4):(nx+5)].=1000
#	K[nx:(Nx-1)*nx, (2*nx):(2*nx+2)].=1000
	K[nx:(Nx-1)*nx, (2*nx+4):(2*nx+5)]=K[nx:(Nx-1)*nx, (2*nx+4):(2*nx+5)]*1000
	coefficient_values_aux = K[1:Nx*nx, 1:Ny*ny]
...
# ╔═╡ 098af1de-685c-4a61-a4b3-c55e94a3b758
neigh[1,1].elements
...
# ╔═╡ aa8915f0-dde1-4b8b-8dfe-ed2e14a494ee
md""" To print the computed eigevalues in each subdomain we can proceed as follows."""

# ╔═╡ 654ce2d4-6355-4874-93fd-a08be9fba1f9
scatter(neigh_eig[2,4].lambda[1:10],markersize=4, label="λ") 
...
plot(reshape(neigh_basis[4,3].cb[:,2],Nx*nx+1,Ny*ny+1),st=:surface,camera=(10,10),color=cgrad(:jet,10, categorical = true))
