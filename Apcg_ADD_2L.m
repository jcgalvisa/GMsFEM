function [x, error, iter, flag, lambdamax, condnumber] = Apcg_ADD_2L(x, b,br, max_it, tol,dom,dom_ov,Nx,Ny,A0,dir)
...
%  error = norm( r ) / bnrm2;  
  ...
  fprintf(' PCG2L residual(%d) = %g\n',iter,error)
  if ( error < tol ) return, end

  for iter = 1:max_it                       % begin iteration

      zlocal  = applay_Aadd_inv(r,dom,dom_ov,Nx,Ny);
      z0=apply_P0NB(r,dom_ov,Nx,Ny,A0,dir);
%      z0=zerodir(z0,dom,Nx,Ny);
% % % %       subplot(2,2,1)
% % % %       plot_vector(zlocal, dom,Nx,Ny,mu,[]); title('zlocal')
% % % %       hold off
% % % %       subplot(2,2,2)
% % % %       plot_vector(z0, dom,Nx,Ny,mu,[]); title('z0')
% % % %       hold off
% % % % pause
      z=zlocal+z0; 
